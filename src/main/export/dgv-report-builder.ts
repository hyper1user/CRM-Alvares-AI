...
// з dgv_month_meta — і ОС зі статусом СЗЧ/АР/200/ПОЛОН не потрапляли
// сюди автоматично. Тепер потрапляють — `punishmentReason`/`punishmentOrder`
// лишаються опціональними overrides.
// v1.4.3: ЗБ ВИДАЛЕНО — зник безвісти не йде в п.6. Боєць у п.1/п.7
// тільки за відомі дні до зникнення (через mapping ЗБ→null дні зі
// статусом ЗБ просто не потрапляють у JOIN attendance × status_types,
// і calculatePeriods природньо обриває періоди).
const SECTION_6_CODES = ['СЗЧ', 'АР', '200', 'ПОЛОН', 'Бух', 'нар']
